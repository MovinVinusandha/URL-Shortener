...
const RegisterPage: React.FC = () => {
  ...
  // Reset oauth loading on mount or bfcache restore (back navigation)
  useEffect(() => {
          ...
          <div className="bg-background border border-border rounded-xl p-8 shadow-xl max-w-md w-full mx-auto">
            {authConfig?.allowRegistration === false ? (
              <div className="text-center space-y-4 py-2">
                ...
                <h1 className="text-xl font-bold tracking-tight text-foreground">Registration Disabled</h1>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Account registration is currently disabled on this instance. If you need access, please contact your administrator.
                </p>
                <div className="pt-3">
                  <button
                    type="button"
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  {/* OAuth Social Buttons (Always visible; grayed out with helpful badge if unconfigured) */}
                  ...
                  <div className="space-y-2.5">
                    ...
                    <button 
                      ...
                      onClick={() => {
                        if (authConfig && !authConfig.googleOAuthEnabled) {
                          toast.error('Google signup is not configured on this instance. Add OAUTH_GOOGLE_CLIENT_ID to your .env file.');
                          return;
                        }
                      ...
                      className={`w-full flex items-center justify-between px-4 py-2 border rounded-lg text-sm font-medium transition-colors ${
                        authConfig && !authConfig.googleOAuthEnabled
                          ...
                          : 'border-border bg-background text-foreground hover:bg-secondary cursor-pointer'
                      }`}
                    >
                    ...
                    <button 
                      ...
                      disabled={loading || oauthLoading !== null}
                      onClick={() => {
                        if (authConfig && !authConfig.githubOAuthEnabled) {
                          toast.error('GitHub signup is not configured on this instance. Add OAUTH_GITHUB_CLIENT_ID to your .env file.');
                          return;
                        }
                        handleOAuthLogin('github');
                      }}
                      className={`w-full flex items-center justify-between px-4 py-2 border rounded-lg text-sm font-medium transition-colors ${
                        authConfig && !authConfig.githubOAuthEnabled
                          ? 'border-border/60 bg-muted/30 text-muted-foreground opacity-60 cursor-not-allowed'
                          : 'border-border bg-background text-foreground hover:bg-secondary cursor-pointer'
                      }`}
                    >
